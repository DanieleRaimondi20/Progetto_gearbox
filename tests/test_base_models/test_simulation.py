...
class OneDofSecondOrderModel(Model):
    ...
    @classmethod
    def from_adimensional_parameters(cls, mass: float, critical_damping: float, natural_frequency: float):
...
def simulate_underdamped_system() -> Simulation:
    ...
    underdamped_model.set_initial_conditions(
...
def simulate_overdamped_system() -> Simulation:
    ...
    overdamped_model.set_initial_conditions(
        init_conditions_dict=initial_conditions
    )
