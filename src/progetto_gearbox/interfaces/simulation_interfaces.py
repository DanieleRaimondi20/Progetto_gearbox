...
import numpy as np
from progetto_gearbox.utils.array_validation import check_column_vector, check_row_vector, check_matrix
...
class Model(ABC):
    ...
    def set_initial_conditions(self, init_conditions_dict: dict[float]):
        self.initial_conditions = np.array(
            [init_conditions_dict[state_name] if (state_name in init_conditions_dict.keys()) else 0 for state_name in self.state_names])
    ...
    def get_inputs(self, time: float | NDArray) -> NDArray:
        inputs = np.array([self.input_funcs[input_name](time) for input_name in self.input_names])
        return inputs
    ...
    def process_equation(self, time: float, state_vector: NDArray) -> NDArray:
        ...
        state_vector_derivative = self.state_transition_matrix @ state_vector + self.input_matrix @ input_vector
        if self.non_linear_process:
            state_vector_derivative += self.non_linear_process(time, state_vector, input_vector)
